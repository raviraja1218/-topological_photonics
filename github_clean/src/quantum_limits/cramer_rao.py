...
Computes fundamental sensitivity limits from Q-factor and design parameters
"""
...
class CramerRaoCalculator:
    ...
    def calculate_standard_quantum_limit(self):
        ...
        N = self.measurement['photon_number']
        T = self.measurement['measurement_time_s']
    ...
    def calculate_quantum_fisher_information(self):
        ...
        Q = self.design['q_factor']
        V = self.design['mode_volume_lambda_cubed']
        gamma = self.design['nonlinear_gamma_W_per_km']
        loss = self.design['loss_dB_per_cm']
    ...
    def calculate_all(self):
        ...
        classical = self.config['comparison_technologies']['classical_ring']
        nv = self.config['comparison_technologies']['nv_center']
        squid = self.config['comparison_technologies']['superconducting']
        
        results = {
            'calculation_time': datetime.now().isoformat(),
            'design_name': self.design['name'],
            'input_parameters': self.design,
...
def main():
    ...
    logging.info(f"Design: Crescent-Moire v1")
    
    calculator = CramerRaoCalculator(args.config)
    results = calculator.calculate_all()
    
    logging.info(f"Q-factor: {results['input_parameters']['q_factor']:,}")
